...
function insertAudit(
  ...
  db.prepare(
    `INSERT INTO audit_log (timestamp, user_id, user_name, action, entity_type, entity_id, entity_name, details, previous_hash, entry_hash)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(ts, userId, userName, action, entityType, entityId, entityName, details, lastAuditHash, hash);
}
...
const insertUser = db.prepare(
  `INSERT INTO users (email, password_hash, full_name, full_name_kana, role, department, title, phone)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
);

for (const u of users) {
  insertUser.run(u.email, passwordHash, u.full_name, u.full_name_kana, u.role, u.department, u.title, u.phone);
}
...
// ============================================================
const contacts = [
  // Nomura RE Master Fund (client 1)
  ...
  // Mitsubishi Estate Logistics REIT (client 5)
  ...
  // CLSA Capital Partners (client 6)
  ...
  // Pacific Century Group (client 8)
...
const insertContact = db.prepare(
  `INSERT INTO client_contacts (client_id, first_name, last_name, title, department, email, phone, is_primary_contact, is_decision_maker, created_by)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`
);

for (const c of contacts) {
  insertContact.run(c.client_id, c.first_name, c.last_name, c.title, c.department, c.email, c.phone, c.is_primary_contact, c.is_decision_maker);
}
...
const insertProduct = db.prepare(
  `INSERT INTO fund_products (product_name, product_name_en, product_type, description, typical_tenor_months, min_amount_jpy, max_amount_jpy, base_rate, spread_bps_min, spread_bps_max)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
);

for (const p of products) {
  insertProduct.run(p.product_name, p.product_name_en, p.product_type, p.description, p.typical_tenor_months, p.min_amount_jpy, p.max_amount_jpy, p.base_rate, p.spread_bps_min, p.spread_bps_max);
}
...
const insertCP = db.prepare(
  `INSERT INTO client_products (client_id, product_id, facility_amount_jpy, spread_bps, start_date, maturity_date, status, created_by)
   VALUES (?, ?, ?, ?, ?, ?, ?, 1)`
);

for (const cp of clientProducts) {
  insertCP.run(cp.client_id, cp.product_id, cp.facility_amount_jpy, cp.spread_bps, cp.start_date, cp.maturity_date, cp.status);
}
...
const interactions = [
  ...
  // SBI Capital Management (client 3)
  ...
  // Tokio Marine Capital (client 4 - prospect)
  ...
  // Daiwa RE Investment (client 7 - dormant)
  ...
  // Pacific Century Group Japan (client 8 - prospect)
  ...
  // Kenedix Investment Partners (client 10)
...
const insertInteraction = db.prepare(
  `INSERT INTO interactions (client_id, interaction_type, subject, description, interaction_date, duration_minutes, location, meeting_objective, meeting_outcome, next_steps, follow_up_date, sentiment, priority, created_by, internal_participants, external_participants, proposal_amount_jpy, proposal_status)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
);
...
const insertBusinessCard = db.prepare(
  `INSERT INTO business_cards (contact_id, client_id, image_path, company_name, person_name, department, title, phone, mobile, email, address, exchange_date, owner_user_id, notes, tags)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
);

for (const bc of businessCards) {
  insertBusinessCard.run(
...
for (const a of auditEntries) {
  insertAudit(a.userId, a.userName, a.action, a.entityType, a.entityId, a.entityName, a.details, a.ts);
}
