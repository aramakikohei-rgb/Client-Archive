...
export const PUT = withAuth(async (request, { user, params }) => {
  const id = Number(params?.id);
  const existing = await queryOne("SELECT * FROM business_cards WHERE id = ?", [id]);
  if (!existing) {
    return NextResponse.json({ error: "名刺が見つかりません" }, { status: 404 });
  }
  ...
  values.push(id);
  await execute(`UPDATE business_cards SET ${updates.join(", ")} WHERE id = ?`, values);
  ...
  const updated = await queryOne("SELECT * FROM business_cards WHERE id = ?", [id]);
  return NextResponse.json(updated);
});

export const DELETE = withAuth(async (request, { user, params }) => {
  const id = Number(params?.id);
  const existing = await queryOne<{ person_name: string; company_name: string }>(
  ...
  await execute("DELETE FROM business_cards WHERE id = ?", [id]);
